...
Or open the orchestrator first and plan interactively:
...
This is a typical orchestration flow.

1. Explore the current implementation when the relevant code path is not yet
   clear.

   ```text
   agent(command="start", agent="code/explore",
         message="Map the authentication module: entry points, middleware chain, and session handling")
   ```

2. Create a todo list and a focused worktree.

   ```text
   write_todos(replace=[
     { description = "Replace session auth with JWT in middleware", status = "pending" },
     { description = "Add JWT token generation endpoint", status = "pending" },
     { description = "Update tests for the JWT flow", status = "pending" }
   ])

   git_worktree(command="create", branch="sia/jwt-refactor")
   ```

3. Start the implementer in the returned worktree path.

   ```text
   agent(command="start", agent="code/implement",
         message="Replace session-based auth with JWT tokens. Use the exploration notes and keep the diff focused.",
         workspace="/path/to/worktree/sia/jwt-refactor")
   ```

4. Review the worktree before reporting back.

   ```text
   git_worktree(command="status", branch="sia/jwt-refactor")
   git_worktree(command="diff", branch="sia/jwt-refactor")

   agent(command="start", agent="code/review",
         message="Review the JWT refactor for correctness and security issues.",
         workspace="/path/to/worktree/sia/jwt-refactor")
   ```

5. Send follow-up instructions if the review finds a problem.

   ```text
   agent(command="send", id=1,
         message="Fix the token expiry issue the reviewer found, then rerun the relevant tests.")
   ```

6. Hand the result back to the user with the branch name and verification
   status.

   ```text
   Changes are ready on branch `sia/jwt-refactor`.
   Verified with: <tests or checks>
   Remaining risks: <if any>
   ```

7. Merge only when the user asks for it. The merge runs through `bash`, so the
   usual confirmation prompt becomes the approval step.

   ```text
   bash(command="start",
        description="Merge JWT refactor branch",
        bash_command="git merge --no-ff sia/jwt-refactor")
   ```

## Parallel Work

You can split a larger task across multiple worktrees.

```text
git_worktree(command="create", branch="sia/feature-a")
git_worktree(command="create", branch="sia/feature-b")

agent(command="start", agent="code/implement",
      message="Implement feature A.",
      workspace="/path/to/worktree/sia/feature-a")

agent(command="start", agent="code/implement",
      message="Implement feature B.",
      workspace="/path/to/worktree/sia/feature-b")
